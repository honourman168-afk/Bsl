...
app.get("/api/leaderboard", (req, res) => {
  const rows = db.prepare(`
    ...
    FROM users ORDER BY points DESC, id ASC
  `).all();
  res.json(rows);
});
...
app.post("/api/admin/matches", auth, (req,res) => {
  ...
  const info = db.prepare(`
    INSERT INTO matches(home_team,away_team,match_time,deadline,home_points,draw_points,away_points)
    VALUES(?,?,?,?,?,?,?)
  `).run(homeTeam.trim(), awayTeam.trim(), matchTime, deadline, Number(homePoints)||0, Number(drawPoints)||0, Number(awayPoints)||0);
